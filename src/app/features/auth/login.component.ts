...
@Component({
  ...
  template: ` <main class="login-shell">
    ...
    <section class="login-card">
      <div
        class="login-status"
        [class.online]="backend() === 'online'"
        [class.offline]="backend() === 'offline'"
      >
      ...
      <form [formGroup]="form" (ngSubmit)="submit()">
        ...
        @if (form.controls.email.touched && form.controls.email.invalid) {
          <span id="email-error" class="field-error">Ingresa un correo válido.</span>
        }
        <label
          >Contraseña<input
            type="password"
        ...
        <button class="primary" [disabled]="form.invalid || loading() || backend() !== 'online'">
        ...
        this.error.set(
          response.status === 401
            ...
            : response.status === 403
              ...
              : response.status === 0
                ? 'No fue posible conectar con el servidor. Reinicia el frontend con npm start.'
